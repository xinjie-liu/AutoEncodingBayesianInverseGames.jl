...
function MCPGame(game, horizon, context_state_block_dimensions = 0)
    ...
    private_constraint_per_player = let
        map(1:num_player) do ii
            ...
            # # TODO: only for two-player one-dim driving example: rear player does not have collision avoidance responsibility
            # if ii == 1
            #     collision_radius = game.env.radius
            #     private_collision_avoidance = map(2:length(xs)) do ii
            #         (xs[ii][Block(1)][1] - xs[ii][Block(2)][1])^2 + (xs[ii][Block(1)][2] - xs[ii][Block(2)][2])^2 - (2*collision_radius)^2
            #     end
            #     append!(private_inequality_constraints, private_collision_avoidance)
            # end
