...
function get_coupling_constraints(game, hypothesis_index)
    if isnothing(game.coupling_constraints)
        return nothing
    end
    x -> game.coupling_constraints(x, hypothesis_index)
end
                ...
                JointStrategy(
                    map(1:num_players(contingency_game)) do player_index
                        ...
                        get_emergency_breaking_strategy(
                            ...
                            get_coupling_constraints(contingency_game, hypothesis_index),
...
function get_emergency_breaking_strategy(
    dynamics::Union{
        TrajectoryGamesExamples.UnicycleDynamics,
        TrajectoryGamesExamples.BicycleDynamics,
    },
    initial_state,
    ...
    function (x, t)
        ...
        if strategy_type === :stop && (
            v >= 0 || (
                !isnothing(coupling_constraints) && any(
                    (
...
function blockedlike(x)
    to_blockvector(only(BlockArrays.blocksizes(x)))
end
...
function get_symbolic_parameter_representation(
    ...
    if parameter isa BlockArrays.BlockArray
        return parameter_symbolic |> blockedlike(parameter)
    end
...
function MCPContingencySolver(
    ...
    planning_horizon_configuration = StaticPlanningHorizon(contingency_game.horizon),
    ...
    hypotheses_symbolic = map(1:number_of_hypotheses) do hypothesis_index
        ...
        cost_per_player = mapreduce(
            ...
            contingency_game.cost.stage_cost(
                ...
                t,
                (;
                    belief.cost_parameters,
                    context_state = context_state_symbolic,
                    emergency_mode = emergency_mode_symbolic,
                ),
            ) .* is_active
        end
...
function ContingencyGames.solve_contingency_game(
    ...
    shared_constraint_premultipliers = [1.0, 1.0],
